...
Tetrislogic::Tetrislogic(sf::RenderWindow& window, sf::Vector2f pos, sf::Vector2i square_size, float scale)
...
void Tetrislogic::update(sf::Time const& deltaTime)
{	
...
void Tetrislogic::tetDirection(direction dir)
{
	click_dx =static_cast<float> (dir); 
}

void Tetrislogic::rotate()
{
...
void Tetrislogic::speed()
{
	mus.play(4);
	delay = 10;
}

void Tetrislogic::lineDead(int g) 
{
	for (int i = g; i > 0; i--)
	{
...
bool Tetrislogic::check(ch ch)
{
	switch (ch)
	{	case Tetrislogic::ch::x:
			{	for (int i = 0; i < 4; i++)
				{if ((t[i].x + click_dx < 0) || 
...
void Tetrislogic::mustet(bool m)
{
	playMus = m;
}

int Tetrislogic::getscore() const
{
	return score;
}

sf::Vector2f Tetrislogic::getPositio()
{
	sf::Vector2f pos;
...
void Tetrislogic::maket(sf::Vector2f posmak)
{
